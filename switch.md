...
allowed-tools: Bash(CLAUDLE_SCRIPT_PATH *), Bash(bash CLAUDLE_SCRIPT_PATH *)
---

User argument: "$ARGUMENTS"

**If no argument was given** — list mode:
1. Run: `bash CLAUDLE_SCRIPT_PATH list` (output: number, family, alias, ID, name, release date).
...
1. Run: `bash CLAUDLE_SCRIPT_PATH switch '<argument>'` (single-quote the argument).
...
3. If the script prints `NO_INJECTION_BACKEND`, this terminal can't inject input. Give the user the resolved command to run themselves: `/model <resolved-id>`. Mention they can use `claudle` to make `/switch` work in any terminal.
